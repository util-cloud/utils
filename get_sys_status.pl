...
$o_unit = "MB";
my $status = 'UNKNOWN';
my  %ERRORS = ( 'UNKNOWN' => 1, 'OK' => 0, 'WARNGING' =>'1', 'CRITICAL'=>2);
...
sub check_file {
	...
	$lxs->init;
		...
		my $fh_crit = $o_critical;
                my $fh_warn = $o_warning;
		my $fh_usage = int($file->{fhalloc} * 100 / 655350);
		if( $fh_usage>=$fh_crit) {
                        $status = "CRITICAL";
                }
		elsif( $fh_usage>=$fh_warn) {
                        $status = "WARNING";
                }
		...
		print "OPEN FILES $status allocated: $file->{fhalloc}";
	}
	else {
                print "No data";
        }
...
sub help {
	...
	ex : 
	Memory usage                    : perl get_sys_status.pl -M -w 90 -c 95
	Net Work usage                  : perl get_sys_status.pl -N -N -w 1000000 -c 60000000 -p eth0
	Cpu usage                       : perl get_sys_status.pl -C -w 90 -c 95 -s 5
	Disk usage                      : perl get_sys_status.pl -D -w 95 -c 100 -u % -p /tmp,/usr,/var
	Load average                    : perl get_sys_status.pl -L -w 10,8,5 -c 20,18,15
	Paging statistics		: perl get_sys_status.pl -W -w 10,1000 -c 20,2000 -s 3
	Process statistics              : perl get_sys_status.pl -P -w 100 -c 200
	I/O statistics on disk device   : perl get_sys_status.pl -I -w 10 -c 5 -p sda1,sda4,sda5,sda6      
	Network usage                   : perl get_sys_status.pl -N -w 10000 -c 100000000 -p eth0
	Processes virtual memory        : perl get_sys_status.pl -T -w 9551820 -c 9551890 -p /var/run/sendmail.pid 
	Uptime				: perl get_sys_status.pl -U -w 5
HELP
}
